...
public class FractalRecursiveApp extends JPanel{
        ...
        while(scan.hasNext()){
            if(scan.hasNextInt()){
            	...
            	if(fracOrder < 0)
            		fracOrder = 0;
                scan.close();
                FractalRecursiveApp fractal = setupJFrame();
                //drawFrac(fracOrder); // draw the fractal
                FractalRecursive fj = new FractalRecursive(fracOrder);
                length = fj.getLength();
    ...
    private static FractalRecursiveApp setupJFrame(){
    	// setup the JFrame
        JFrame frame = new JFrame("Fractal Jigsaw");
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        FractalRecursiveApp fractal = new FractalRecursiveApp();
